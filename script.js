...
function normalizeItem(raw) {
  ...
  return {
    ...
    demoStudyPlan: raw.demoStudyPlan || null,
...
function priorityLabel(index, total) {
  if (index <= Math.max(1, Math.floor(total * 0.25))) return 'High';
  if (index <= Math.max(2, Math.floor(total * 0.6))) return 'Medium';
  return 'Low';
}

function buildDemoStudyPlan(item) {
  const due = toDate(item.dueDate);
  const today = new Date();
  const days = Math.max(2, Math.ceil((due - today) / 86400000));
  const count = Math.min(6, Math.max(3, days - 1));
  const start = addDays(due, -count);
  const baseTopics = inferTopics(item).slice(0, 6).map((t) => t.name);
  const topics = baseTopics.length ? baseTopics : ['Core material review', 'Practice session'];
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const priority = priorityLabel(i + 1, count);
    const duration = isAssignment
      ? (i === count - 1 ? '60 min' : i < 2 ? '90 min' : '75 min')
      : (i === count - 1 ? '120 min' : i % 3 === 0 ? '105 min' : '90 min');
    const task = isAssignment
      ? (i < 2 ? `Research + outline for ${topic}` : i < count - 1 ? `Draft/revise sections for ${topic}` : 'Final revision and submission check')
      : (i < count - 1 ? `Study and practice ${topic}` : 'Final review + mixed practice set');
    planDays.push({ date, topic, task, duration, priority });
  }

  const hardTopics = topics.slice(0, 3);
  const strategyTips = isAssignment
    ? [
      'Front-load research and outline work so drafting is faster.',
      'Leave one dedicated revision pass before submission.',
      'Use a short final checklist for formatting and requirements.',
    ]
    : [
      'Review weak topics earlier and revisit them after practice.',
      'Alternate concept review with timed practice problems.',
      'Do one final mixed session the day before the exam.',
    ];

  const focusAreas = topics.slice(0, 4).map((name, idx) => ({
    name,
    priority: priorityLabel(idx + 1, topics.length),
  }));

  const sourceHint = item.studyInputs.topics.text || item.studyInputs.notes.text || item.aiAnalysis?.summary
    ? 'uploaded materials and notes'
    : 'title, type, and date';
  ...
  return {
    ...
    summary: isAssignment
      ? `Assignment plan generated from ${sourceHint}. It prioritizes research, drafting, revising, and a final quality pass.`
      : `Exam plan generated from ${sourceHint}. It balances topic review, practice problems, and a final confidence review.`,
    days: planDays,
    focusAreas,
    hardTopics,
    strategyTips,
...
function renderDemoStudyPlan(plan) {
  if (!plan) return '<p class="helper">Generate a plan to show a polished day-by-day study schedule.</p>';
  return `
    <div class="card">
      <b>Plan summary</b>
      <p class="helper">${escapeHtml(plan.summary)}</p>
    </div>
    <div class="list" style="margin-top:10px">
      ${plan.days.map((d, i) => `
        <div class="card">
          <b>Day ${i + 1} · ${d.date}</b>
          <p>${escapeHtml(d.task)} <span class="badge">${escapeHtml(d.duration)}</span> <span class="badge priority-${d.priority.toLowerCase()}">${d.priority}</span></p>
          <p class="helper">Focus area: ${escapeHtml(d.topic)}</p>
        </div>
      `).join('')}
    </div>
    <div class="grid-2" style="margin-top:10px">
      <div class="card">
        <b>Estimated focus areas</b>
        <ul>${plan.focusAreas.map((f) => `<li>${escapeHtml(f.name)} <span class="badge priority-${f.priority.toLowerCase()}">${f.priority}</span></li>`).join('')}</ul>
      </div>
      <div class="card">
        <b>Topics that may take longer</b>
        <ul>${plan.hardTopics.map((t) => `<li>${escapeHtml(t)}</li>`).join('')}</ul>
      </div>
    </div>
    <div class="card" style="margin-top:10px">
      <b>Best strategy tips</b>
      <ul>${plan.strategyTips.map((tip) => `<li>${escapeHtml(tip)}</li>`).join('')}</ul>
    </div>
  `;
}
...
function renderStudyMode(item) {
  if (state.studyMode === 'plan' && (item.type === 'exam' || item.type === 'assignment')) {
    return `
      <div class="card">
        <div class="actions" style="justify-content:space-between;align-items:center">
          <b>AI Study Plan Generator</b>
          <button class="btn primary" data-act="generate-demo-plan">Generate Study Plan</button>
        </div>
        ${renderDemoStudyPlan(item.demoStudyPlan)}
      </div>
    `;
  }
...
function wireGlobalHandlers() {
  ...
  document.body.addEventListener('click', (e) => {
    ...
    if (act === 'generate-demo-plan') {
      const item = state.items.find((i) => i.id === state.selectedStudyItemId);
      if (!item) return;
      item.demoStudyPlan = buildDemoStudyPlan(item);
      upsertItem(item);
      return;
    }
  });
